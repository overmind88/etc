...
package body AWS.POP is
   ...
   begin
      Net.Buffered.Put_Line (Mailbox.Sock, "UIDL " & Utils.Image (N));
      ...
      begin
         ...
         if Response (1 .. 3) /= "+OK" then
            raise POP_Error;
         else

            for I in 6 .. Response'Last loop
               if Response (I) = ' ' then
                  return Natural'Value (Response (I + 1 .. Response'Last));
               end if;
            end loop;
